...
DSI Studio starts the current AI agent in this DSI Studio AI directory, where `dsi.cmd` is located, and adds the user-selected project directory separately for project access. DSI Studio supplies `DSI_STUDIO_AGENT` and `CODEX_THREAD_ID`; `dsi.cmd` supplies them to the relay and applies a process-scoped PowerShell execution-policy bypass.
