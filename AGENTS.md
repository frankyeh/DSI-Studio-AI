...
Always include `bash` before `./dsi.sh`. If this launcher does not work, read `DSI_STUDIO_AI_LAUNCHER.md` for platform requirements, troubleshooting, and the Windows `./dsi` fallback.
...
```bash
bash ./dsi.sh TITLE "<concise title derived from the user's task>"
```

## 2. Send a progress message

```bash
bash ./dsi.sh CHAT "<brief improvised message that you are learning the DSI Studio relay>"
```
...
bash ./dsi.sh tracking<hex-address> set_slice <slice-index>
bash ./dsi.sh tracking<hex-address> list_slice
```
...
```bash
bash ./dsi.sh tracking<hex-address> list_unet
bash ./dsi.sh tracking<hex-address> segment_brain human_synthseg <slice-index>
...
bash ./dsi.sh tracking<hex-address> add_region_from_atlas "0 1 3&4"
bash ./dsi.sh tracking<hex-address> list_region
```
...
The same folder contains `DSI_STUDIO_AI_MANUAL.md` for relay rules, `DSI_STUDIO_AI_SKILL_*.md` for task workflows, and `DSI_STUDIO_AI_COMMAND_EXAMPLES_*.md` for command syntax and inventories. Read only the files relevant to the user's task, then continue using `bash ./dsi.sh`.
