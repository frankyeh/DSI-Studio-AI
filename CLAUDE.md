...
```powershell
./dsi_claude.ps1 TITLE "<concise title derived from the user's task>"
```

## 2. Send a progress message

```powershell
./dsi_claude.ps1 CHAT "<brief improvised message that you are learning the DSI Studio relay>"
```
...
```powershell
./dsi_claude.ps1 main open_fib "C:/data/subject.fz"
./dsi_claude.ps1 LIST
```
...
```powershell
./dsi_claude.ps1 main list_recent_fib
./dsi_claude.ps1 main open_fib "<exact relevant path returned by list_recent_fib>"
./dsi_claude.ps1 LIST
```
...
```powershell
./dsi_claude.ps1 tracking<hex-address> list_slice
./dsi_claude.ps1 tracking<hex-address> set_slice <slice-index>
./dsi_claude.ps1 tracking<hex-address> list_slice
```
...
```powershell
./dsi_claude.ps1 tracking<hex-address> list_unet
./dsi_claude.ps1 tracking<hex-address> segment_brain human_synthseg <slice-index>
./dsi_claude.ps1 tracking<hex-address> list_region
```
...
```powershell
./dsi_claude.ps1 tracking<hex-address> list_atlas
./dsi_claude.ps1 tracking<hex-address> add_region_from_atlas "0 1 3&4"
./dsi_claude.ps1 tracking<hex-address> list_region
```
...
```powershell
./dsi_claude.ps1 tracking<hex-address> list_param tracking
```

### 5.2 Start tracking

```powershell
./dsi_claude.ps1 tracking<hex-address> run_tracking "Whole Brain"
```

### 5.3 Wait for completion

```powershell
./dsi_claude.ps1 tracking<hex-address> list_tract status
```
...
```powershell
./dsi_claude.ps1 tracking<hex-address> list_tract
```
...
```powershell
./dsi_claude.ps1 main hub_repo
```

### 6.2 List tags

```powershell
./dsi_claude.ps1 main hub_tags "<exact repository returned by hub_repo>"
```

### 6.3 List matching files

```powershell
./dsi_claude.ps1 main hub_files "<exact repository>" "<exact tag returned by hub_tags>" ".fz" 0 20
```
...
```powershell
./dsi_claude.ps1 main hub_open "<exact repository>" "<exact tag>" <file-row-index>
./dsi_claude.ps1 LIST
```
...
The same folder contains `DSI_STUDIO_AI_MANUAL.md` for relay rules, `DSI_STUDIO_AI_SKILL_*.md` for task workflows, and `DSI_STUDIO_AI_COMMAND_EXAMPLES_*.md` for command syntax and inventories. Read only the files relevant to the user's task, then continue using `./dsi_claude.ps1`.